...
# This build includes:
#   • S1 predictor (unchanged core, but fixed non-contiguous view / inplace)
#   • Contextual τ-policy with proper REINFORCE update (EMA baseline + entropy bonus)
...
#   • Basis embedding is now a registered Parameter (no lazy params that optimizer misses)
# ============================================================
...
class TargetTransform:
    ...
    def inverse_torch(self, t: torch.Tensor) -> torch.Tensor:
        ...
        if self.kind == "logit01":
            ymin = float(0.0 if self.ymin is None else self.ymin)
            ymax = float(1.0 if self.ymax is None else self.ymax)  # FIX: use stored ymax
...
class S1Predictor(nn.Module):
    def __init__(self, in_dim: int, hidden: int = 256, depth: int = 3, dropout: float = 0.0):
        ...
        for _ in range(depth):
            layers += [nn.Linear(d, hidden), nn.ReLU(inplace=False), nn.LayerNorm(hidden)]
    ...
    def forward(self, x):  # x: (B,N,F)
        B,N,F = x.shape
        return self.net(x.reshape(B*N, F)).reshape(B, N)  # FIX: reshape handles non-contiguous
...
class TauPolicyContextual(nn.Module):
    ...
    NOTE: Basis embedding is a single learnable vector (this script runs one basis at a time).
    """
    def __init__(self, ctx_dim: int, hid: int = 64, basis_dim: int = 8):
        super().__init__()
        self.basis_vec = nn.Parameter(torch.empty(basis_dim))
        nn.init.normal_(self.basis_vec, std=0.05)
        self.mlp = nn.Sequential(
            nn.Linear(ctx_dim + basis_dim, hid), nn.ReLU(inplace=False),
            nn.Linear(hid, hid), nn.ReLU(inplace=False),
            nn.Linear(hid, 2)  # [mu, log_sigma]
        )
    def forward(self, ctx: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        B = ctx.size(0)
        x = torch.cat([ctx, self.basis_vec.view(1,-1).expand(B,-1)], dim=1)
    ...
    def sample_tau(self, ctx: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        mu, sigma = self.forward(ctx)
        eps = torch.randn_like(mu)
        tau = mu + sigma * eps  # rsample-like
        return tau, mu, sigma
...
class Trainer:
    ...
    def _train_tau_for_branch(self, branch: str):
        ...
        entropy_coef = 1e-3  # small entropy bonus to avoid sigma collapse
        ...
        for ep in range(1, self.args.rl_epochs+1):
            ...
            for batch in dl:
                ...
                # predict s1 and context (no grad)
                ...
                tau_sample, mu_tau, sigma_tau = self.tau_ctx_policy.sample_tau(ctx_t)
                # clamp for selection (no grad through clamp path)
                tau_val = float(min(max(float(tau_sample.item()), self.args.tau_lower_hard), self.args.tau_upper_hard))

                # selection by literal threshold and ASF evaluation
                try:
                ...
                # RL objective: L_total = (E_dmrg - E0)^2 + size penalty
                k = len(act_used0)
                L_e = (float(E_dmrg.item()) - float(E0))**2
                L_s = self.args.size_penalty * float((k - int(self.args.k0_target))**2)
                L_total = L_e + L_s

                # REINFORCE with EMA baseline + entropy regularization
                self._opt_tau_ctx.zero_grad()
                dist = Normal(mu_tau, sigma_tau)
                # log_prob of the unclamped sample
                logp = dist.log_prob(tau_sample).mean()
                b = baseline.update(L_total)
                advantage = float(L_total - b)
                # Loss to MINIMIZE: (L - b) * log_prob - entropy_coef * entropy
                loss = torch.as_tensor(advantage, device=self.device) * logp - entropy_coef * dist.entropy().mean()
                loss.backward()
                ...
                used += 1
                self.logger.info(
                    "[RL-τctx] [%s] ep=%02d [%d/%d] idx=%d name=%s | tau=%.6f | k=%d | E=%.10f Ha | L=%.6e | b=%.6e",
                    branch, ep, step_idx, M, meta['idx'], meta['name'], tau_val, k, float(E0), float(L_total), float(b)
                )
    ...
    def evaluate(self, branches: List[str], tag="TEST",
        ...
        for batch in dl:
            ...
            per_branch = {}
            for b in branches:
                # use deterministic τ = mu(ctx) at eval
                mu_tau, sigma_tau = self.tau_ctx_policy.forward(ctx_t)
...
def build_cache_from_xyz_one_stab(
    ...
    for rec in ds.records:
        ...
        key_data = {"basis": basis, "charge": rec.charge, "spin2S": rec.spin2S,
                    "atoms": [(a, [float(f"{x:.8f}") for x in xyz]) for a, xyz in zip(rec.atoms, rec.coords)]}  # FIX f-string
        blob = json.dumps(key_data, sort_keys=True, separators=(",", ":"))
        key = hashlib.sha256(blob.encode("utf-8")).hexdigest()

        used_persist = False
        if persist_cc_path and key in persist_map:
            try:
